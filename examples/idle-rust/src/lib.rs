...
mod tests {
    ...
    #[test]
    fn parses_gpu_layers() {
        let args = parse_args(["-m", "model.gguf", "-ngl", "0"]).unwrap();
    ...
    fn rejects_unexpected_argument() {
        assert_eq!(
            parse_args(["-m", "model.gguf", "-c", "512"]).unwrap_err(),
            ParseError::UnexpectedArgument("-c".to_string())
        );
    }
}
