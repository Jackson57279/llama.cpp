...
// DFlash2 selector: top-k candidates per block position plus pairwise scores,
// packed into the nextn output slot for the CPU-side walk.
...
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
    ...
    // DFlash2 selector reads these logits in-graph, so apply the target's output transforms here
